...
class URL
{
    ...
    */
    ...
    public static function getItemUrl($item, $is_ss)
    {
        ...
        if (!$is_ss) {
            if(!empty($item['tunnel_server'])){
                $tunnel = explode(":",$item['tunnel_server']);
            }

            if(!empty($tunnel)){
                $ssurl = $tunnel[0] . ':' . $tunnel[1] . ':' . $item['protocol'] . ':' . $item['method'] . ':' . $item['obfs'] . ':' . Tools::base64_url_encode($item['passwd']) . '/?obfsparam=' . Tools::base64_url_encode($item['obfs_param']) . '&protoparam=' . Tools::base64_url_encode($item['protocol_param']) . '&remarks=' . Tools::base64_url_encode($item['remark']) . '&group=' . Tools::base64_url_encode($item['group']);
            }else{
